...
//创建视图
func (handler *Handler) handleCreateView(opt, txid string) (*pb.QueryResponse, error) {
	var respChan chan pb.ChaincodeMessage
	var err error
	if respChan, err = handler.createChannel(txid); err != nil {
		return nil, err
	...
	payloadBytes, _ := proto.Marshal(&pb.ViewOpt{Opt: []byte(opt)})
	msg := &pb.ChaincodeMessage{Type: pb.ChaincodeMessage_CREATE_VIEW, Payload: payloadBytes, Txid: txid}
	
	var responseMsg pb.ChaincodeMessage
	if responseMsg, err = handler.sendReceive(msg, respChan); err != nil {
		return nil, errors.New(fmt.Sprintf("[%s]error sending %s", shorttxid(msg.Txid), pb.ChaincodeMessage_CREATE_VIEW))
	}
	
	if responseMsg.Type.String() == pb.ChaincodeMessage_RESPONSE.String() {
		// Success response
		chaincodeLogger.Debugf("[%s]Received %s. Successfully create view", shorttxid(responseMsg.Txid), pb.ChaincodeMessage_RESPONSE)
