...
#################################################################################
    ...
    def runIt(self, command, scriptName):
        cmd = "/opt/gopath/src/github.com/oxchains/fabric/test/regression/daily/chaincodeTests/%s %s %s %s %s %s"% (scriptName, self.CHANNEL_NAME, self.CHANNELS, self.CHAINCODES, self.ENDORSERS, command)
